import { Building2, Flame, Mail, MapPin, Phone, Search, ShieldAlert, Siren, UsersRound } from "lucide-react";
...
export default function OperatorDirectory({ apiBaseUrl, token }) {
  ...
  return <section className="panel personnel-directory" id="personnel-directory">
    <div className="panel-header directory-header"><div><h2>Connected Personnel &amp; Departments</h2><p>Verified Police, Fire, Rescue and Hospital network</p></div><label><Search size={14}/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search name, station or area"/></label></div>
    ...
    {!visible.length && <p className="directory-empty"><UsersRound size={20}/>{message || "No matching verified personnel found."}</p>}
  </section>;
}
